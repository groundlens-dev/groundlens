...
class TestEvaluateBatch:
    ...
    def test_non_string_question_raises(self) -> None:
        with pytest.raises(ValueError, match="question must be a non-empty string"):
            evaluate_batch([{"question": None, "response": "A."}])  # type: ignore[dict-item]

    def test_non_string_response_reports_item_index(self) -> None:
        items = [
            {"question": "Q1?", "response": "A1."},
            {"question": "Q2?", "response": 123},  # type: ignore[dict-item]
        ]
        with pytest.raises(ValueError, match="Item 1"):
            evaluate_batch(items)
